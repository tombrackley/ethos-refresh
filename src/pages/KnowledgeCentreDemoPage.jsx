...
function KnowledgeCard({ item, onClick, isVisited, isCompleted }) {
  ...
  const typeLabel = ALL_TYPE_LABELS[item.type] || item.type
...
export default function KnowledgeCentreDemoPage() {
  ...
  const [showFavourites, _setShowFavourites] = useState(false)
  ...
  const clearFilters = () => { setActiveTypes([]); setActiveSkills([]); setActiveGoals([]); setSearch('') }

  const hasFilters = activeTypes.length > 0 || activeSkills.length > 0 || activeGoals.length > 0 || search.trim()
  ...
  const filteredItems = useMemo(() => {
    ...
    return items
  }, [activeTypes, activeSkills, activeGoals, search, showFavourites, bookmarked])
  ...
  return (
    <div className="flex flex-1 overflow-hidden bg-white">
      <div className="flex-1 overflow-auto relative">
        ...
        <div className="pointer-events-none absolute inset-x-0 -top-2 -left-2 -right-2 h-[480px] bg-gradient-to-b from-[#C8EEFF]/20 to-transparent" />

        {/* Header + filters */}
        <div className="max-w-7xl mx-auto px-6 pt-14 pb-6 space-y-5 relative">
          <div>
            <h1 className="text-2xl font-semibold text-foreground">Knowledge Centre</h1>
            <p className="text-sm text-muted-foreground mt-1">Structured professional knowledge and learning materials</p>
          </div>
          ...
          <div className="relative">
            ...
            <Input
              ...
              className="pl-12 pr-10 h-11 text-sm rounded-lg bg-white border-border"
            />
            {search && (
        ...
        <div className="max-w-7xl mx-auto px-6 pb-6 space-y-8">
          ...
          {hasFilters && (
            <div className="flex flex-wrap items-center gap-2">
              {activeSkills.map(s => (
                <span key={s} className="flex items-center gap-1.5 text-xs font-medium text-foreground border border-border bg-white px-2.5 h-7 rounded-full">
                  {s}
                  <X className="size-3 cursor-pointer text-muted-foreground hover:text-foreground" onClick={() => toggleSkill(s)} />
                </span>
              ))}
              {activeTypes.map(t => (
                <span key={t} className="flex items-center gap-1.5 text-xs font-medium text-foreground border border-border bg-white px-2.5 h-7 rounded-full">
                  {ALL_TYPE_LABELS[t] || t}
                  <X className="size-3 cursor-pointer text-muted-foreground hover:text-foreground" onClick={() => toggleType(t)} />
                </span>
              ))}
              ...
              <div className="bg-white border-b border-[rgba(229,229,229,0.6)] shrink-0">
                ...
                <div className="px-5 py-4 space-y-2">
                  <div className="flex flex-wrap items-center gap-1.5">
                    ...
                    <span className="inline-flex items-center text-xs font-medium text-muted-foreground bg-[#f5f5f5] rounded-[6px] px-2 h-6">{ALL_TYPE_LABELS[selectedItem.type] || selectedItem.type}</span>
                  </div>
